...
import {
  ...
  type CreatePpBaseRequestInput,
  type SavePpDetailsInput,
  type SubmitPpRequestInput,
...
function getPpRequestDelegate() {
  const clientWithPp = prisma as PrismaClient & {
    ppRequest?: PrismaClient["ppRequest"];
  };
  return clientWithPp.ppRequest;
}

export async function createPpBaseRequest(
  input: CreatePpBaseRequestInput
): Promise<ActionResult<{ requestId: string; requestNo: string }>> {
  const validatedInput = createPpBaseRequestSchema.safeParse(input);
  ...
  for (let attempt = 0; attempt < 5; attempt += 1) {
    try {
      ...
      return { success: false, message: "Failed to create PP base request." };
    }
  }
...
export async function savePpDetails(
  input: SavePpDetailsInput
): Promise<
  ActionResult<{
    projectId: string;
    projectCode: string;
...
> {
  const validatedInput = savePpDetailsSchema.safeParse(input);
  if (!validatedInput.success) {
    return {
      success: false,
  ...
  const payload = validatedInput.data;
  const ppRequestDelegate = getPpRequestDelegate();
  if (!ppRequestDelegate) {
    return {
      success: false,
      message: "PP persistence is not initialized yet. Please restart server.",
    };
  }
  ...
  try {
    ...
    await ppRequestDelegate.upsert({
    ...
    if (error instanceof Prisma.PrismaClientKnownRequestError) {
      return {
        success: false,
        message: "Unable to save PP project details due to a database constraint issue.",
      };
    }
    return { success: false, message: "Failed to save PP project details." };
  }
}

export async function submitPpRequest(
  input: SubmitPpRequestInput
): Promise<ActionResult<{ requestId: string; requestNo: string }>> {
  const validatedInput = submitPpRequestSchema.safeParse(input);
  if (!validatedInput.success) {
    return {
      success: false,
  ...
  try {
    ...
    if (!request) {
      return {
        success: false,
        message: "Request was not found. Please restart the PP form.",
      };
    }
    if (!pp || !pp.projectId) {
      return {
        success: false,
        message: "PP details are incomplete. Please review Step 2 and try again.",
      };
    }

    await prisma.$transaction([
      ppRequestDelegate.update({
        where: { requestId: payload.requestId },
        data: {
    ...
    return { success: false, message: "Failed to submit PP request." };
  }
}

...
import {
  VAP_ALLOWED_DOCUMENT_MIME_TYPES,
  VAP_FORM_CODE,
  VAP_MAX_DOCUMENT_SIZE_BYTES,
  VAP_REQUEST_TITLE,
  createVapBaseRequestSchema,
  saveVapDetailsSchema,
  submitVapRequestSchema,
} from "@/lib/validations/vap";
import { createVapBaseRequest, saveVapDetails, submitVapRequest } from "../_actions/vap";
import { VAP_STEPPER_STEPS, type ProjectOption, type RequestorContext, type UploadedAssetState, type VapDetailsState } from "../_config/vap-form-schema";

type FieldErrors = Record<string, string[]>;
type AlertState = { type: "success" | "error" | "info"; message: string } | null;
type VapFormProps = { channel: "gcpc" | "gcp"; requestor: RequestorContext; projects: ReadonlyArray<ProjectOption>; canSubmitRequest: boolean; };

type PersistedVapState = {
  currentStep: number;
  requestId: string | null;
  requestNo: string | null;
  details: VapDetailsState;
  uploadedDocument: UploadedAssetState | null;
  acknowledgement: boolean;
};

const VAP_SESSION_STORAGE_KEY = "gcp-central:form:vap:v1";
const MAX_FILE_SIZE_MB = VAP_MAX_DOCUMENT_SIZE_BYTES / (1024 * 1024);
const acceptedDocumentTypes = VAP_ALLOWED_DOCUMENT_MIME_TYPES.join(",");

export default function VapMultiStepForm({ channel, requestor, projects, canSubmitRequest }: VapFormProps) {
  ...
  const [details, setDetails] = useState<VapDetailsState>({
    projectId: "",
    projectCode: "",
  ...
  useEffect(() => {
    const persisted = readPersistedFormState<PersistedVapState>(VAP_SESSION_STORAGE_KEY);
    if (persisted) {
      setCurrentStep(Math.min(Math.max(persisted.currentStep, 1), VAP_STEPPER_STEPS.length));
  ...
  useEffect(() => {
    ...
    writePersistedFormState<PersistedVapState>(VAP_SESSION_STORAGE_KEY, {
      currentStep,
      requestId,
      requestNo,
      details,
  ...
  async function handleFileUpload(event: ChangeEvent<HTMLInputElement>) {
    ...
    if (!file) return;
    resetFeedback();
    if (!VAP_ALLOWED_DOCUMENT_MIME_TYPES.includes(file.type as (typeof VAP_ALLOWED_DOCUMENT_MIME_TYPES)[number])) {
      setErrorState("Unsupported file type.");
      return;
    }
    if (file.size <= 0 || file.size > VAP_MAX_DOCUMENT_SIZE_BYTES) {
  ...
  function handleCreateBaseRequest() {
    resetFeedback();
    const payload = {
      requestType: VAP_FORM_CODE,
      routingType: category,
      requestTitle: VAP_REQUEST_TITLE,
      category,
    ...
    const validated = createVapBaseRequestSchema.safeParse(payload);
    if (!validated.success) return setErrorState("Please review the pre-populated basic information fields.");
    startTransition(async () => {
      const result = await createVapBaseRequest(validated.data);
  ...
  function handleSaveStep2() {
    ...
    const validated = saveVapDetailsSchema.safeParse(payload);
    if (!validated.success) return setErrorState("Please correct project details.");
    startTransition(async () => {
      const result = await saveVapDetails(validated.data);
      if (!result.success) return setErrorState(result.message, result.fieldErrors);
      setCurrentStep(3);
  ...
  function handleFinalSubmit() {
    ...
    const validated = submitVapRequestSchema.safeParse(payload);
    if (!validated.success) return setErrorState("Please complete acknowledgement and document requirements.");
    startTransition(async () => {
      const result = await submitVapRequest(validated.data);
      if (!result.success) return setErrorState(result.message, result.fieldErrors);
      setIsSubmitted(true);
      clearPersistedFormState(VAP_SESSION_STORAGE_KEY);
      setAlertState({ type: "success", message: `VAP request ${result.data.requestNo} submitted successfully.` });
  ...
  return (
    <div className="surface-card p-5 sm:p-6">
      <MultiStepStepper steps={VAP_STEPPER_STEPS} currentStep={currentStep} isSubmitted={isSubmitted} />
      {alertState ? (
      ...
      {currentStep === 1 ? (
        <section className="space-y-4">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <InputField label="Request Title" value={VAP_REQUEST_TITLE} readOnly inputClassName="bg-slate-50" />
      ...
      {currentStep === 3 ? (
        <section className="space-y-4">
          ...
          <div className="flex flex-wrap justify-between gap-3 pt-2">
            ...
            {canSubmitRequest ? <Button type="button" onClick={handleFinalSubmit} loading={isBusy || isSubmitted}>{isSubmitted ? "Submitted" : isBusy ? "Submitting..." : "Submit VAP"}</Button> : null}
          </div>
        </section>
      ) : null}

...
export type RppFormSchemaContext = {
  requestTitle: string;
  category: "GCP" | "GCPC";
...
export const RPP_FORM_STEPS: ReadonlyArray<
  ConfigDrivenStepDefinition<RppFormSchemaContext>
> = [
  {
  ...
  },
  {
    ...
    description:
      "Select the project for this revised procurement plan. Project code and company are auto-filled.",
    fieldsContainerClassName: "grid gap-4 md:grid-cols-2",
    fields: [
      {
        id: "project",
...
export const RPP_STEPPER_STEPS = RPP_FORM_STEPS.map((step) => ({
  id: step.id,
  label: step.label,
}));
